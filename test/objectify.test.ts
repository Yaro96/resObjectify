...
describe("objectify", () => {
  ...
  describe("combinedFields", () => {
    ...
    it("includes multi-key groups with null components when allowNulls=true", () => {
      const rows = [
        { eventType: "click", country: "US", count: 10 },
        { eventType: "view", country: null, count: 99 },
        { eventType: "click", country: undefined, count: 100 },
      ];
      ...
      expect(objectify(rows, fields)).toEqual([
        {
          events: [{ eventType: "click", country: "US", counts: [10] }],
        },
      ]);

      expect(objectify(rows, fields, { allowNulls: true })).toEqual([
        {
          events: [
            { eventType: "click", country: "US", counts: [10] },
            { eventType: "view", country: null, counts: [99] },
            { eventType: "click", country: undefined, counts: [100] },
          ],
        },
      ]);

      expect(objectify(rows, fields, { object: true, allowNulls: true })).toEqual({
        events: {
          "click-US": { eventType: "click", country: "US", counts: [10] },
          "view-null": { eventType: "view", country: null, counts: [99] },
          "click-undefined": { eventType: "click", country: undefined, counts: [100] },
        },
      });
    });
  ...
  describe("groups", () => {
    ...
    it("handles group without fields at root level", () => {
    ...
    it("includes null-key groups when allowNulls is set on a nested group", () => {
      const rows = [
        { id: 1, rule_id: 1, formula: "x" },
        { id: 1, rule_id: null, formula: "y" },
        { id: 1, rule_id: undefined, formula: "z" },
      ];

      const fields: Field[] = ["id", ["rules", ["rule_id", "formula"]]];
      const fieldsNulls: Field[] = [
        "id",
      ...
      expect(objectify(rows, fieldsNulls)).toEqual([
        {
          ...
          rules: [
            { rule_id: 1, formula: "x" },
            { rule_id: null, formula: "y" },
            { rule_id: undefined, formula: "z" },
          ],
        },
      ]);

      expect(objectify(rows, fieldsNulls, { object: true, allowNulls: false })).toEqual({
        1: {
          id: 1,
          rules: {
            1: { rule_id: 1, formula: "x" },
            null: { rule_id: null, formula: "y" },
            undefined: { rule_id: undefined, formula: "z" },
          },
        },
      });
    });
    ...
    it("respects flattenSingleField option", () => {
      const rows = [
        { id: 1, rule_id: 10, formula: "x" },
        { id: 1, rule_id: 20, formula: "y" },
        { id: 2, rule_id: 10, formula: "z" },
      ...
      const fields: Field[] = ["id", ["rules", ["formula"]]];
      ...
      expect(objectify(rows, fields, { object: true, flattenSingleField: false })).toEqual({
        1: { id: 1, rules: { x: { formula: "x" }, y: { formula: "y" } } },
        2: { id: 2, rules: { z: { formula: "z" } } },
      });

      const fields2: Field[] = [
        "id",
        [{ name: "rules", flattenSingleField: false }, ["formula", { key: "id", hide: true }]],
      ];

      expect(objectify(rows, fields2, { flattenSingleField: true })).toEqual([
        { id: 1, rules: [{ formula: "x" }, { formula: "y" }] },
        { id: 2, rules: [{ formula: "z" }] },
      ]);
    });
